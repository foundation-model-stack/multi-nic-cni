...
In addition to [the common MultiNicNetwork with IPVLAN-L3](#common-multinicnetwork), `Multi-NIC CNI` offers unique benefits tailored to each specific cloud infrastructure, as shown below. Corresponding network options (MultiNicNetwork) for each infrastructure are listed accordingly.
